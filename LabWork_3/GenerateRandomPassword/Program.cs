...
namespace GenerateRandomPassword
{
    class Program
    {
        ...
        private static string[] options = { "smallLetters", "capitalLetters", "numbers", "specialCharacters" };
        ...
        private static void SelectingOptions()
        {
            ...
            do
            {
                for (int i = 0; i < 4; i++)
                {
                    selectedOption[i] = Input($"Do you want to include {options[i]} (y/n)? ").ToLower();
